...
public class DefaultAgentAdapter implements AgentAdapter {
  ...
  // unit bytes
  private static final String UDP_MESSAGE_SIZE = "65535";
  // unit kilo bytes
  ...
  public boolean init(String[] agents, ProtocolType protocolType, boolean compress) {
    ...
    for (String agent : agents) {
      tmp = agent.split(":");
      if (protocolType == ProtocolType.UDP) {
        prx = AgentPrxHelper.uncheckedCast(ic.stringToProxy("A:udp -h " + tmp[0] + " -p " + tmp[1])
            .ice_locatorCacheTimeout(60).ice_compress(compress).ice_datagram());
      } else if (protocolType == ProtocolType.TCP) {
        prx = AgentPrxHelper.uncheckedCast(ic.stringToProxy("A:tcp -h " + tmp[0] + " -p " + tmp[1]).ice_compress(
            compress));
      }
      prxList.add(prx);
    }
