...
public class DefaultWatcherManager implements WatcherManager {
  ...
  @Override
  public void setSlotWatcher(int soltSize, long startTime)
      throws KeeperException, InterruptedException, IOException {
    for (int i = 0; i < soltSize; i++) {
      conn.get().getChildren(
          clusterPath + "/" + i,
          new SlotWatcher(conn, clusterPath, i, new DefaultHABalancer(conn,
              clusterPath, soltSize), startTime));
    }
  }

  @Override
  public void setDispatcherWatcher(String currentZnode, Watcher watcher)
      throws KeeperException, InterruptedException, IOException {
    ...
    List<String> childrenList = conn.get().getChildren(
        clusterPath + "/" + slot, false);
  ...
  @Override
  public void setClientWatcher(int soltSize,
      DispatcherClientManager clientManager) throws KeeperException,
