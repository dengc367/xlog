...
public abstract class HDFSSyncTask extends Thread{
  ...
  public HDFSSyncTask(File slaveLogFile,int slaveLogRootDirLen){
    this.categories=slaveLogFile.getParent().substring(
  ...
  public void run(){
    ...
    try {
      remoteFs=getRemoteFS(conf);
    } catch (IOException e) {
      LOG.error("Fail to remote FileSystem  instance,then it dose not sync slave log : "+slaveLogFile.getAbsolutePath(),e);
      return ;
    }
    try {
      if(FileUtil.copy(localFs,new Path(slaveLogFile.toString()), remoteFs,new Path(categories+"/"+slaveLogFile.getName()+".recover"),false,conf)){
        rename(slaveLogFile);
      }
    } catch (IOException e) {
  ...
  protected abstract FileSystem getRemoteFS(Configuration conf) throws IOException;
