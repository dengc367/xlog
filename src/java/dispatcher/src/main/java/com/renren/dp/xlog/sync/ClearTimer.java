...
public class ClearTimer extends TimerTask {
  ...
  public ClearTimer(int localStorageCacheTime, int errorDataCacheTime) {
    ...
    errorDataLogDir = new File(storePath + "/"
  ...
  public void run() {
    if (localStorageCacheTime > 0) {
      CacheLogFileFilter opLogCLFF = new CacheLogFileFilter(
          localStorageCacheTime);
    ...
    if (errorDataCacheTime > 0) {
      CacheLogFileFilter storageCLFF = new CacheLogFileFilter(
          errorDataCacheTime);
